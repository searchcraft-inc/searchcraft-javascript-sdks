...
| `viewAllResultsBaseUrl` | `view-all-results-base-url` | Base URL for the "View all results" footer link. The current search term will be appended (URL encoded).  For example, in a CMS-backed site you might set this to `/?s=` so the final URL becomes `/?s=<search-term>`. | `string \| undefined` | `undefined` |
| `viewAllResultsLabel` | `view-all-results-label` | Optional label for the "View all results" footer button. Defaults to "View All Results". | `string \| undefined` | `undefined` |
