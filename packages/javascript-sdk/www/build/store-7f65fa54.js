...
var d = (o, t, r) => t in o ? a(o, t, { enumerable: !0, configurable: !0, writable: !0, value: r }) : o[t] = r;
var s = (o, t, r) => d(o, typeof t != "symbol" ? t + "" : t, r);
class g {
  constructor(t) {
    s(this, "config");
    /**
     ...
     */
    s(this, "search", async (t) => {
      if ((t.query.match(/"/g) || []).length % 2 !== 0)
        throw new Error(
          "The search term contains an uneven number of quote characters."
        );
      try {
        const e = this.config.index.join(","), c = `${this.config.endpointURL}/index/${e}/search`, i = {
          query: {
            [t.mode]: { ctx: t.query }
          },
          limit: 20
        };
        t.order_by && (i.order_by = t.order_by, i.sort = t.sort === "desc" || t.sort === "asc" ? t.sort : "asc");
        const u = {
          method: "POST",
          headers: {
        ...
        if (console.log("REQUEST RESPONSE", n), !n.ok)
          throw new Error(
            `Error: ${n.statusText} (Status: ${n.status})`
          );
        return await n.json();
      } catch (e) {
        throw console.error("Error parsing response:", e), e;
      }
    });
...
class f {
  ...
  defaultFormatter(t, r) {
    return `[${l[t]}] ${( new Date()).toISOString()}: ${r}`;
  }
  debug(t) {
...
const useSearchcraftStore = zustand.create((set, get) => {
    ...
    return {
        ...
        search: async () => {
            ...
            log(l.INFO, `Starting search with query: "${query}"`);
            setIsRequesting(true);
            try {
                const results = await searchcraft.search({ query, mode: 'fuzzy' });
                setSearchResults(results);
                log(l.DEBUG, `Search results: ${JSON.stringify(results)}`);
            }
            catch (error) {
                console.error(`Search failed: ${error.message}`);
                log(l.ERROR, `Search failed with error: ${error.message}`);
            }
            finally {
        ...
        initialize: (searchcraftInstance, debug = false) => {
            ...
            debuggerInstance = debug
                ? new f({ logLevel: l.DEBUG })
                : null;
            log(l.INFO, 'Searchcraft store initialized.');
        },
    };
});
...
//# sourceMappingURL=store-7f65fa54.js.map