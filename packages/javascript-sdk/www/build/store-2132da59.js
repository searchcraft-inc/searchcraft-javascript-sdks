...
var r$1 = (o, t, e) => g(o, typeof t != "symbol" ? t + "" : t, e);
class a {
  constructor(t) {
    r$1(this, "config");
    /**
     ...
     */
    r$1(this, "search", async (t) => {
      if (console.log("SEARCH PARAMS", t), (t.query.match(/"/g) || []).length % 2 !== 0)
        throw new Error(
          "The search term contains an uneven number of quote characters."
        );
      try {
        const n = this.config.index.join(","), c = `${this.config.endpointURL}/index/${n}/search`, i = {
          query: {
            [t.mode]: { ctx: t.query }
          },
          limit: 20
        };
        t.order_by && (i.order_by = t.order_by, i.sort = t.sort === "desc" || t.sort === "asc" ? t.sort : "asc"), console.log("REQUEST BODY", i);
        const u = {
          method: "POST",
          headers: {
        ...
        return console.log("REQUEST RESPONSE", s), await s.json();
      } catch (n) {
        return n;
      }
    });
    if (!t.endpointURL || !t.index)
      throw new Error("Endpoint URL and Index value(s) must be provided");
    this.config = t;
  }
}
var l$1 = /* @__PURE__ */ ((o) => (o[o.DEBUG = 0] = "DEBUG", o[o.INFO = 1] = "INFO", o[o.WARN = 2] = "WARN", o[o.ERROR = 3] = "ERROR", o))(l$1 || {});
class f {
  constructor(t) {
    r$1(this, "logLevel");
    r$1(this, "logFormatter");
    this.logLevel = t.logLevel || 1, this.logFormatter = t.logFormatter || this.defaultFormatter;
  }
  defaultFormatter(t, e) {
    return `[${l$1[t]}] ${( new Date()).toISOString()}: ${e}`;
  }
  debug(t) {
...
const useSearchcraftStore = zustand.create((set, get) => {
    ...
    return {
        ...
        search: async () => {
            ...
            log(l$1.INFO, `Starting search with query: "${query}"`);
            setIsRequesting(true);
            try {
                const results = await searchcraft.search({ query, mode: 'fuzzy' });
                console.log(results);
                setSearchResults(results);
                log(l$1.DEBUG, `Search results: ${JSON.stringify(results)}`);
            }
            catch (error) {
                console.error(`Search failed: ${error.message}`);
                log(l$1.ERROR, `Search failed with error: ${error.message}`);
            }
            finally {
        ...
        initialize: (searchcraftInstance, debug = false) => {
            ...
            debuggerInstance = debug
                ? new f({ logLevel: l$1.DEBUG })
                : null;
            log(l$1.INFO, 'Searchcraft store initialized.');
        },
    };
});
...
export { a, useThemeStore as b, useSearchcraftStore as u };

//# sourceMappingURL=store-2132da59.js.map