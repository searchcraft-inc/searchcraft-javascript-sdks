/* eslint-disable */
/* tslint:disable */
/* auto-generated vue proxies */
import { defineContainer } from '@stencil/vue-output-target/runtime';

import type { JSX } from '@searchcraft/javascript-sdk';

export const SearchcraftFilterPanel =
  /*@__PURE__*/ defineContainer<JSX.SearchcraftFilterPanel>(
    'searchcraft-filter-panel',
    () => {},
    ['searchcraftId', 'items', 'responsiveBreakpoint', 'responsiveBehavior'],
  );

export const SearchcraftInputForm =
  /*@__PURE__*/ defineContainer<JSX.SearchcraftInputForm>(
    'searchcraft-input-form',
    () => {},
    [
      'searchcraftId',
      'autoSearch',
      'buttonPlacement',
      'buttonLabel',
      'inputLabel',
      'placeholderValue',
      'placeholderBehavior',
      'value',
      'inputFocus',
      'inputBlur',
      'inputInit',
    ],
    ['inputFocus', 'inputBlur', 'inputInit'],
  );

export const SearchcraftPagination =
  /*@__PURE__*/ defineContainer<JSX.SearchcraftPagination>(
    'searchcraft-pagination',
    () => {},
    ['searchcraftId'],
  );

export const SearchcraftPopoverButton =
  /*@__PURE__*/ defineContainer<JSX.SearchcraftPopoverButton>(
    'searchcraft-popover-button',
    () => {},
    ['template', 'type', 'searchcraftId'],
  );

export const SearchcraftPopoverForm =
  /*@__PURE__*/ defineContainer<JSX.SearchcraftPopoverForm>(
    'searchcraft-popover-form',
    () => {},
    [
      'type',
      'popoverResultMappings',
      'hotkey',
      'hotkeyModifier',
      'searchcraftId',
      'placeholderValue',
      'placeholderBehavior',
    ],
  );

export const SearchcraftResultsInfo =
  /*@__PURE__*/ defineContainer<JSX.SearchcraftResultsInfo>(
    'searchcraft-results-info',
    () => {},
    ['template', 'searchcraftId'],
  );

export const SearchcraftSearchResults =
  /*@__PURE__*/ defineContainer<JSX.SearchcraftSearchResults>(
    'searchcraft-search-results',
    () => {},
    ['searchcraftId', 'initialQuery', 'template'],
  );

export const SearchcraftSearchResultsPerPage =
  /*@__PURE__*/ defineContainer<JSX.SearchcraftSearchResultsPerPage>(
    'searchcraft-search-results-per-page',
    () => {},
    ['increment', 'searchcraftId'],
  );

export const SearchcraftSummaryBox =
  /*@__PURE__*/ defineContainer<JSX.SearchcraftSummaryBox>(
    'searchcraft-summary-box',
    () => {},
    ['searchcraftId'],
  );

export const SearchcraftTheme =
  /*@__PURE__*/ defineContainer<JSX.SearchcraftTheme>(
    'searchcraft-theme',
    () => {},
  );
